<template>
    <div class="flex w-full flex-col justify-end">
        <div class="flex h-10 justify-around space-x-2">
            <save-queue />
            <button v-for="command in playbackCommands" :key="command.icon" :aria-label="command.command"
                @click="Command(command.command)" :class="[
            ...
            <button aria-label="setting" @click="toggleMediaController"
    ...
    <div class="flex h-28 w-full justify-around text-lightest md:h-24">
        <button aria-label="previous-song" @click="Command('previous')" class="scale-110 hover:text-accent">
            <font-awesome-icon icon="step-backward" size="2x" />
        </button>
        <like-song :pLiked="liked" :file="currentSong.file" />
        <button aria-label="toggle-playback" @click="Command('toggle')" :class="[
            ...
            <font-awesome-icon :icon="['fas', status.state === 'play' ? 'pause' : 'play']" size="lg" />
        </button>
        <button aria-label="stop-song" @click="Command('stop')" class="scale-110 hover:text-accent">
            <font-awesome-icon icon="stop" size="2x" />
        </button>
        <button aria-label="next-song" @click="Command('next')" class="scale-110 hover:text-accent">
